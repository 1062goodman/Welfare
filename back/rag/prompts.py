...
[현재까지 추천된 정책 목록]
{current_recommedations}
사용자가 "1번", "두 번째 거", "청년도약계좌" 등으로 특정 정책을 지칭하면, 위 목록과 대조하여 target_policy를 정확히 추출하세요. 목록이 비어있다면 사용자가 지칭할 대상이 없다는 뜻입니다.
...
반드시 아래에 제공된 [검색 데이터]를 바탕으로 사용자의 질문에 답변하세요.
